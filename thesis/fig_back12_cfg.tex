\begin{tikzpicture}[baseline=(mult.center)]
  ...
  \node[stmt, below=0.25in of assign] (test) {!+i < cnt+!\labelNode{fig_back11_test}};
  \node[labelfor=test] () {\refNode{fig_back11_test}};

  \node[stmt, below of=test] (mult) {!+n += val * m+!\labelNode{fig_back11_mult}};
  \node[labelfor=mult] (mult_label) {\refNode{fig_back11_mult}};

  \node[stmt, below of=mult] (incr) {!+i+++!\labelNode{fig_back11_incr}};
  \node[labelfor=incr] () {\refNode{fig_back11_incr}};
  
  \node[stmt, below of=incr] (return) {!+return n+!\labelNode{fig_back11_return}}; 
