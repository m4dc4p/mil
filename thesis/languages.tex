...
alternatives for three reasons. The memory behavior of call-by-need
can be hard to predict (i.e., the problem of ``space leaks'' in lazy
languages is well known). Call-by-value is the norm among popular
high-level languages (Scheme, ML, and JavaScript, to name a few). Our
research language, Habit \citep{HabitXX}, designed for systems and
device programming, also uses the call-by-value strategy, motivating
our choice.

Figure~\ref{lang_fig4} uses Plotkin's ``structural operational
semantics'' \citep{PlotkinXX} to presents \emph{rewrite rules} for the
...
matches a redex to the left-hand side of one of our rules. The redex
is rewritten according to the right-hand side. If no more redexes
remain, evaluation terminates. Each rewrite is called a
\emph{transition}, and the strategy as a whole a \emph{transition
  function}.
...
\begin{minipage}{2.5in}
  \begin{math}
    \begin{array}{lclr}
      \lamApp{t_1}{t_2} & \rightarrow & \lamApp{t_1}{v} & \text{\sc CBN-App1} \\
      \lamApp{t_1}{v_2} & \rightarrow & \lamApp{v_1}{v_2} & \text{\sc CBN-App2} \\
      \lamAppP{\lamAbs{x}{t}}{v} & \rightarrow & [v \mapsto x] t & \text{\sc CBN-Abs}
    \end{array}
  \end{math}
\end{minipage}
...
We now show how natural numbers, and some common arithmetic operations, can be 
expressed using just the \lamA. We start with the \emph{Church Numerals}, that 
represent the natural numbers (0, 1, 2, etc.). Each numeral is represented
as some number of successors of 0::
\begin{equation}
  \begin{split}
...
\begin{equation*}
  \begin{array}{rlr}
    ...
    &= \lamAbs{s}{\lamAbs{z}{\lamApPp{s}{\lamApp{0}{\lamApp{s}{z}}}}} & \text{\sc E-Abs} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApPp{s}{\lamAppP{\lamAbs{s}{\lamAbs{z}{z}}}{\lamApp{s}{z}}}}} & \text{\emph{Definition of 0 from Equation~\ref{lang_eq_nats}}} \\
    & \multicolumn{2}{r}{\emph{For illustration, we reduce the $\lambda$'s introduced by 0, though normally we would not.}} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApPp{s}{\lamAppP{\lamAbs{z}{z}}{z}}}}  & \text{\sc E-Abs} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{s}{z}}}  & \text{\sc E-Abs} \\
...
\begin{equation*}
  \begin{array}{llr}
    ...
    &= \lamAppP{\lamAbs{n}{\lamAbs{s}{\lamAbs{z}{\lamApp{1}{\lamApPp{s}{\lamApp{n}{\lamApp{s}{z}}}}}}}}{1} & \text{\sc E-Abs} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{1}{\lamApPp{s}{\lamApp{1}{\lamApp{s}{z}}}}}} & \text{E-Abs} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{(\lamAbs{s}{\lamAbs{z}{\lamApp{s}{z}}})}{\lamApPp{s}{\lamApp{1}{\lamApp{s}{z}}}}}} & \text{\emph{Definition of 1 from Equation~\ref{lang_eq_nats}}.} \\
    & \multicolumn{2}{r}{\emph{Again, we reduce under the $\lambda$ here, for illustration.}} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{(\lamAbs{z}{\lamApp{s}{z}})}{(\lamApp{1}{\lamApp{s}{z}})}}} & \text{\sc E-abs} \\

    &= \lamAbs{s}{\lamAbs{z}{\lamApp{s}{(\lamApp{1}{\lamApp{s}{z}})}}} & \text{\sc E-abs} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{s}{(\lamApp{(\lamAbs{s}{\lamAbs{z}{\lamApp{s}{z}}})}{\lamApp{s}{z}})}}} & \text{\emph{Definition of 1.}} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{s}{(\lamApp{(\lamAbs{z}{\lamApp{s}{z}})}{z})}}} & \text{\sc E-Abs}} \\
    &= \lamAbs{s}{\lamAbs{z}{\lamApp{s}{\lamApp{s}{z}}}} & \text{\sc E-Abs}} \\
