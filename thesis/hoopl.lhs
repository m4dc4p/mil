...
\citeyearpar{Kildall1973}, it makes the composition of multiple
dataflow analyses much simpler. We return to Lerner and colleagues'
work in Section~\ref{hoopl_sec9}.
...
the representation of programs and facts, a transfer function, a rewriter, and a
meet operator.
...
only assigning to dead variables). Our running example will implement a
...
gives an overview of the types, data structures, and functions
provided by \hoopl. Sections~\ref{hoopl_sec_cfg} through
\ref{hoopl_sec6} give detailed information about each
item. Throughout, we develop our client program to implement dead-code
elimination. We conclude with a summary of \hoopl in
Section~\ref{hoopl_sec3}.

\section{\Hoopl's API}
\label{hoopl_sec1}

\intent{Introduce \hoopl-managed structures} To implement dataflow
analysis generically, \hoopl defines several core data structures that
client programs must use. These include the representation of \cfgs,
the types of transfer and rewrite functions, and the implementation of
the meet operator. \Hoopl controls the \cfg representation so it can
traverse, propagate facts around, and rewrite the \cfg. \Hoopl
specifies the type of the transfer and rewrite function such that they
produce usable information (and rewrites). Finally, \hoopl specifies
the form of the meet operator (but not its implementation) so that the
library can recognize fixed points.
...
expression or statement from the \ast of the language that the client
...
\hoopl determines when an analysis reaches a fixed point, \hoopl relies on
the client to specify when one set of facts equals another. Finally,
...
\emph{content} and \emph{shape}. Content takes the form of statements
or expressions from the client's \ast, while shape specifies how
control-flow enters and leaves a given block. An ``open'' block allows
control-flow to fall-through implicitly from its predecessor or to
fall-through to its successor. A ``closed'' block requires that
control-flow explicitly transfers to or from the block. Shape
constrains the \cfg such that only blocks with compatible shapes can
be connected: predecessors of an open block must be open; successors
of a closed block must be closed.
...
explicitly transfer to a successor block on exit. An |O O| block
requires a unique predecessor and a unique successor; this allows
...
|x| (``entry'' and ``exit'') types for each constructor, reflecting
the control-flow of the represented statement. The |CExpr| and |Var|
types do not affect control-flow in our subset, so we do not annotate
them like |CStmt|. \Hoopl defines the |Label| type; we use it to
define the successors and predecessors of closed blocks.
...
a block defined outside the program. In this way |Call| acts like an
...
\intent{Introduce \hoopl functions for building graphs.}  \Hoopl
defines the type |Graph| for representing \cfgs.
Figure~\ref{hoopl_fig4} shows the five primitive functions that \hoopl
provides for client programs to use for constructing \cfgs. The |n|
type in each primitive's signature represents the \ast defined by the
client program (|CStmt| in our example).

\intent{Introduce |mkFirst|, |mkMiddle|, and |mkLast|.}  The
|mkFirst|, |mkMiddle|, and |mkLast| functions turn a single block into
a graph of one block with the same shape. The |(<*>)| operator,
pronounced ``concat,'' connects an ``open on exit'' (|e O|) graph to
an ``open on entry'' (|O x|) graph. The resulting graph's shape, |e
x|, combines the entry shape of the first argument and the exit shape
of the second. Necessarily, the graph represented by the first
argument becomes the predecessor of the graph represented by the
second argument. For example, if |n1| has type |CStmt C O| and |n2|
has type |CStmt O O|, then |n1 <*> n2| would have type |CStmt C O| and
|n1| will be the unique predecessor to |n2| in |n1 <*> n2|. \Hoopl
...
\noindent \Hoopl defines the |Label| type, which the client program
uses for two purposes: uniquely identifying each block and specifying
the explicit successors of a given block. \Hoopl use |entryLabel| method
to find the entry point for a given block. The |n C x| type of its
argument ensures that |entryLabel| can only be applied to ``closed on
entry'' nodes: precisely those nodes that can be the target of an
explicit control-flow transfer. Similarly, \hoopl uses |successors| to
determine the explicit successors of a ``closed on exit'' block.
...
use that instance to recover potential control-flow between basic blocks in
...
\noindent We define |entryLabel| for |Entry|, the only ``closed on
entry'' constructor for |CStmt|.  Similarly, we just define
|successors| for |Return|, the only ``closed on exit'' |CStmt|
...
output facts correspond to the \outBa set for the block.\footnote{A 
  backwards analysis reverses this correspondence.} The first
...
block. Together, the set of possible facts and the meet
operator must form a \emph{lattice}.

\intent{Introduce |DataflowLattice| type and show connection to facts
  and the meet operator.}  \Hoopl provides the |DataflowLattice| type
(shown in Figure~\ref{hoopl_fig7}), which defines the following
fields: |fact_name|, used for documentation; |fact_bot|, for
specifying initial facts; and |fact_join|, for the implementation of
the analysis' meet operator.\footnote{The \hoopl authors choose to
  document their library in terms of \emph{joins}. We follow
  \citep{Aho2006} and use the meet.}
...
$B$; therefore, we implement our meet operator as \emph{set union}: to
compute \outBa for block $B$, we take the union of all the \inE sets
of $B$'s successors.
...
variables that are live at the beginning of each block, \inBa, using the
transfer function defined in Section~\ref{hoopl_sec5}) and \outBa, the
block's input set. Both \inBa and \outBa are subsets of
...
that implements the definitions of the meet operator and facts just
...
the \ast of the program under analysis). The |f| parameter represents the
...
be applied to values with different types (e.g., |Assign| has type
...
\item[|transfer (Assign var expr) f|] In this case, |f| represents the
  ...
  as assignment eliminates live variables. The auxiliary function
  |uses| computes the live variables in |expr|; we add those variables
  to our updated |f| and return the result.
  
\item[|transfer (Call _ exprs) f|] This case resembles
...
\item[|transfer Return f|] No variables (in our \ast) will be
    live after the procedure returns. Therefore, nothing is live at this
    point, and we return the empty set.
\end{description}
...
iteration, and at each label, \hoopl computes the meet of facts from the
...
variation of the dataflow algorithm that addresses these
concerns. Kildall's dataflow algorithm computes facts over a static
\cfg; Lerner and colleagues' algorithm repeatedly transforms the \cfg
\emph{during} analysis. After transforming some or all of the \cfg,
their algorithm re-computes facts for the new \cfg; the transformation
and analysis of the \cfg continues until reaching a fixed point.
...
describe, their algorithm removes the need to combine individual
dataflow analyses manually. Instead, each dataflow analysis can be
implemented separately; their algorithm composes those separate pieces
automatically. \Hoopl implements a version of the interleaved analysis
and rewriting algorithm just described.
...
functions. A rewrite function receives the node to rewrite as its first
...
Rewriters can delete |O O| nodes by returning |Just
...
and |analyzeAndRewriteFwd|, the \hoopl functions that the client program
...
\begin{myfig}[hbpt]\disableoverfull
  \begin{minipage}{\hsize}\begin{withHsLabeled}{hoopl_bwd}\disableoverfull
...
> analyzeAndRewriteBwd :: (CheckpointMonad m, NonLocal n, LabelsPtr entries) => {-"\hslabel{constraints}"-}
>   BwdPass m n f {-"\hslabel{bwdpass}"-}
>   -> MaybeC e entries {-"\hslabel{maybec}"-}
>   -> Graph n e x {-"\hslabel{graph}"-}
>   -> Fact x f {-"\hslabel{fact}"-}
...
function takes a number of arguments and must be run
inside a \hoopl-specified monad. We address each argument in turn.

\begin{description}
\item[|(CheckpointMonad m, NonLocal n, LabelsPtr entries)| --- Line~\ref{hoopl_bwd_constraints}] These
  constraints reflect several \hoopl requirements:
  \begin{itemize}
...
\item[|BwdPass m n f| --- Line~\ref{hoopl_bwd_bwdpass}] This argument packages the client's
definitions of the lattice, transfer function, and rewrite function
for this particular analysis.

\item[|MaybeC e entries| --- Line~\ref{hoopl_bwd_maybec}] This gives all the entry points to the
program, which may not always be all the |Labels| in the \cfg. 

\item[|Graph n e x| --- Line~\ref{hoopl_bwd_graph}] This argument holds the \cfg to be
optimized. In practice, |e x| is always |C C|. 

\item[|Fact x f| --- Line~\ref{hoopl_bwd_fact}] This argument gives the initial input facts for
