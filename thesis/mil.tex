...
\footnote{The values are not necessarily stored in the
  immediate environment. For example, static links might be
...
when creating new functions. In Section~\ref{mil_subsec5} we discuss how
they are used to apply a function to an argument.

\subsection{Creating Functions}
\label{mil_subsec4}

A closure always results when a new function is created. For
...
Since we applied |compose| to only two arguments,
Expression~\eqref{mil_eq1} results in a function value, which we
assign to the variable $t_1$. The notation \lamSubst{a}{f} means we
substitute the value $a$ for the argument $f$. This notation lets us
keep track of the original argument name, as well as the value we
supplied for it in this instance. In Expression~\eqref{mil_eq1}, we
say $x$ is \emph{bound}, because it is given as an argument, and that
$f$ and $g$ are \emph{free} because they are not
arguments. Expression~\eqref{mil_eq5} attaches the label
\textsc{CompL} to the body of |compose|, suggesting that |compose|
appears at fixed location in our program.

When the compiler generates code to evaluate the |compose| function,
it will ensure that the runtime environment (i.e., the closure) is
constructed such that the function can execute with the correct
arguments. One strategy the compiler can use is to put the function's
arguments in the same location within the closure, simplifying the
code in the function's body for locating arguments.

Figure~\ref{mil_fig4} shows the closure represented by $t_1$ and the
original definition of |compose|.  The arrows from $f$ and $g$ in the
definition of |compose| show how the body of the function refers to
fixed locations in the closure. The $x$ argument does not have an
arrow since our term has only captured $f$ and $g$.


\begin{myfig}
  \begin{tabular}{c}
    \begin{minipage}{4in}\vbox to 1in {\vss\hbox to \textwidth{\hss
      \begin{tikzpicture}[remember picture, overlay]
        ...
        \matrix (closure1)
                {
                  \node[clo,minimum height=3\ht\clobox] (compclo_l) {CompL}; & \node[clo,minimum height=3\ht\clobox] (compclo_f) {$f: a$}; & \node[clo,minimum height=3\ht\clobox] (compclo_g) {$g: b$}; \\
                };
      \end{tikzpicture}\hss}\vss}
    \end{minipage} \\
      \tikz[remember picture,overlay]{\node[invis] (comp_l) {}; \draw[->] (comp_l) -- (compclo_l.south);}\text{\sc CompL}:\ %%
      \lamAbs{f}{\lamAbs{g}{\lamAbs{x}{%%
            \lamApp{\tikz[remember picture,overlay]{\node[invis] (comp_f) {}; \draw[->] (comp_f) -- (compclo_f.south);}f}%%
                   {(\lamApp{\tikz[remember picture,overlay]{\node[invis] (comp_g) {}; \draw[->] (comp_g) -- (compclo_g.south);}g}%%
                     {x})}}}}.
  \end{tabular}
  \caption{A function value and its closure.}
  \label{mil_fig4}
\end{myfig}
