\begin{tabular}{lr}
  ...
  \begin{tikzpicture}[baseline=(b1.north)]
    ...
    \node[invis,overlay,below right=1in and -.2in of b1] () {$\{\var t/\,:\,\mkclo[k2:g]\unskip\}, \{\var u/\,:\,\top\}, \{\var f/\,:\,\mkclo[k1:]\unskip\}$};

    \node[invis,overlay,below right=1.8in and -.2in of b1] () {\mfun{out}(\lab b3/): $\{\var f/\,:\,\mkclo[k1:]\unskip\}, \{\var g/\,:\,\mkclo[k4:v]\}$};

    \draw [->] (b1) to (b2);
    \draw [->] (b2) to (b3);
    \pausemd{\draw [->] (b3.south) to ($(b3.south) + (0mm, -.25in)$) -| ($(b2.west) + (-.25in, 0mm)$) to (b2.west);}
  \end{tikzpicture} \\\\
  \hfil\scap{uncurry_loop_r1_a} & \scap{uncurry_loop_r1_b}\hfil
\end{tabular}
