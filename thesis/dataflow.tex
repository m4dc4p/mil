...
Equation~\eqref{eqn_back4} considers two kinds of statements:
assignment and increment. If the statement #a = C# appears in the
...
so \outB{lst_back18_mult} contains \factC{n}{\top}. However, \outB{lst_back18_mult}
also shows that #m# is not modified in \refNode{lst_back18_mult}; the value
from \inB{lst_back18_mult} is just copied to \outB{lst_back18_mult}.

\afterpage{\clearpage\input{fig_back9}\clearpage}

%% The
%% transfer function is one the parameters of the algorithm Each
%% node can have its own unique function, but in practice 

%% Constant propagation is a \emph{forwards analysis}: a node's \inE set is
%% calculated using its predecessors \out sets. This also means a node's
%% \out set is calculated using its \inE set 

%% Not
%% only does this mean we compute a given node's \inE set from its predecessors \out
%% sets, but also that we compuet the same node's \out set using its \inE set.

%% Our analysis computes \out sets for each node in the control-flow
%% graph. A node's \out set is calculated using the statements in the node
%% and its \inE set. We define a \emph{transfer function} for our 
%% analysis that performs this calculation. 
