...
program point. Each \emph{fact} indicates our knowledge of a
...
and $x$ a value in \setLC. Initially, we do not have any information
about the value assigned to any variables, so all the facts associate
$\bot$ with each variable.
...
variables in each \out set of $B$'s predecessors. When we allow \lub
to operate over sets of pairs, we can show how
\outB{lst_back17_assign} \lub \outB{lst_back17_incr} is equivalent to
\inB{lst_back17_test} in Figure~\ref{fig_back11}:
\begin{equation}
  \begin{split}
