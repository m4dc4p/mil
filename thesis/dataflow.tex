...
Consider Figure~\ref{fig_back7}, Part~\subref{fig_back_7_initial}, which
...
\begin{align}
  \inBa &= \bigwedge\limits_{P \in \mathit{pred}(B)} \outXa{P} \\ \notag\\
  B \bigwedge\ \,\mathclap{\emptyset}\phantom{C} &= B \notag\\
  B \bigwedge\ \,\mathclap{C}\phantom{C} &= \bigcup\limits_{\scriptstyle(a,x) \in B}
                    \left(\bigcup\limits_{\scriptstyle(b,y) \in C} (a,x) \wedge (b,y)\right) \\ \\
  (a,x) \wedge (b,y) &= 
  \begin{cases}
    a=x \lub y &, (a,x) \in B, (b,y) \in C,\ \text{\lub as in Table~\ref{tbl_back4}.}\\
    a=x &, b \not\in \mathit{var}(B). \\
    b=y &, a \not\in \mathit{var}(C). \\
  \end{cases} \\ \notag\\
  \setLC &= \bot, 0, {-1}, 1, {-2}, 2, \dots, \top. \notag\\
