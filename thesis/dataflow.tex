...
$\bot$, by Equation~\eqref{eqn_back13}, there is no such $x_2$ such
that $x_2 \sqlt \bot$. Therefore, $x_3$ cannot be $\bot$. If $x_3$ is
an integer, again by Equation~\eqref{eqn_back13}, $x_2$ must be
...
algorithm.\footnote{A \emph{backwards} algorithm is very similar}
...
\begin{myfig}
  \begin{minipage}{3in}
    \begin{AVerb}[numbers=left,gobble=6]
      ...
      \textbf{until} ($\text{Out(B)}^{i} == \text{Out(B)}^{i - 1}$, for all $B$)
