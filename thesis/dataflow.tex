...
can branch to either \refNode{lst_back2_true} (when $\mathit{rnd}() + a > b$) or
\refNode{lst_back2_false} (when $mathit{rnd}() + a \leq b$). A node followed by
...
\begin{myfig}[tbh]
  \begin{tabular}{cc}
    \subfloat{\input{lst_back11}\label{fig_back_7_initial}} & %%
...
\runin{Iteration \& Fixed Points}

\runin{Dataflow Equations}
