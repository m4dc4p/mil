...
is \emph{parameterized} by four items: facts, the meet operator,
...
\eqref{eqn_back13}, and \eqref{eqn_back16} define our meet
operator. These equations and the set \setLC define our lattice. By
calculating \out from \inE, Equation~\eqref{eqn_back3} indicates that
this is a \emph{forwards} analysis.\footnote{Note that this equation
  is the same for all \emph{forward} dataflow analysis'.}
Equations~\eqref{eqn_back15} and \eqref{eqn_back16} define our
transfer function.
...
parameters:
\begin{align*}
  \setL{Facts} &= \text{\it our set of facts}. \\
  \wedge &= \text{\it Our \emph{meet} operator.} \\
  D &= \text{\it Direction, \emph{forwards} or \emph{backwards}}. \\
  \mathit{transfer}(v) &= \text{\it Our transfer function, with $v \in \setL{Facts}$.}
\end{align*}

In turn, we can define the iterative dataflow algorithm for the
\emph{forwards} direction. Figure~\ref{fig_back14} gives the
algorithm.\footnote{A \emph{backwards} algorithm is very similar} The
algorithm initializes all \out sets to $\bot$, or some suitable
initial value from \setL{Fact}. The entry node gets special treatment
in some cases, so we set \outXa{Entry} to its own initial value (though in
many cases \outXa{Entry} is set to the same value as other \out
sets). Line~\ref{fig_back14_loop} gives our termination condition:
when \out sets stop changing, we are done. The superscript on each
\out set represents the iteration. $\outBa^{i}$ means the $i$-th
iteration, and $\outBa^{i-1}$ the previous ($i - 1$) iteration. In our
loop, we calculate all \inE sets from their predecessors' \out sets. The new
\inE sets are used to calculate \out sets for each node. Those \out sets are 
tested and, if they have not changd from the previous iteration, the 
  ...
  \begin{minipage}{3in}
    \begin{AVerb}[numbers=left,gobble=6]
      ...
      \textbf{until} ($\text{Out(B)}^{i} == \text{Out(B)}^{i - 1}$, for all $B$)\label{fig_back14_loop}
        In(B) = $\bigwedge_{P \in pred(B)} \text{Out}(P)$.
        Out(B) = \emph{transfer}(In(B)).
    \end{AVerb}
  \end{minipage}
...
Aho \citep{AhoXX} shows that for a dataflow analysis defined with a
finite lattice and monotone transfer function, Figure~\ref{fig_back14}
will compute a \emph{maximum fixed point}. The result has the property
that, for all $F = \outBa$, no other $F' = \outBa$ can be computed such
that $F \sqlt F'$. In other words, Figure~\ref{fig_back14} will compute
the maximum possible information for the given analysis. 

The maximum fixed point differs from the \emph{ideal} in that it may
make more conservative estimates than justified by the program. In
particular, the algorithm does not consider branches that will never
be taken. For example, the C program from Figure~\ref{fig_back1_a}
will never execute Line~\ref{lst_back19_test_true}, because the test
#if(a > b)# is always false:
\begin{center}
...
solution because it takes into account all actual (and only those)
paths taken by the program at runtime. Determining the actual paths
taken is an undecidable problem -- thus we settle for the maximum
fixed point. Fortunately, the algorithm is conservative --- it never
ignores (or adds) paths --- so we can be sure that its analysis will
never be wrong, just that it probalby will not be as good as the
ideal.
