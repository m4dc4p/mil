...
analysis. A \emph{known integer constant}, $C \in Z$, means our analysis identified
...
annotated with \emph{facts} before (\inE) and after (\out) a few
program points. For this analysis, our facts are sets of pairs,
$(a,x)$, where $a$ is the name of a variable and $x$ a value in
\setLC. Each \emph{fact} indicates our knowledge of a variable's value
...
have no information, such as \factC{a}{\bot}, from those with
non-constant values. This matches the definition of the C language,
where a variable that is not initialized has an undefined value. This
allows us to legally transform some programs by assuming the undefined
variable has the best possible value. For example,
Figure~\ref{fig_back15} shows a block with two
predecessors. \outB{fig_back15_b1} has the fact
\factC{x}{1}, while \outB{fig_back15_b2} says
\factC{x}{\bot}. Since \outB{fig_back15_b2} adds no
information about $x$, we assume it is the best possible value and use
\lub to determine that \inB{fig_back15_b3} should have
...
\begin{myfig}
  \begin{tikzpicture}
    ...
    \node[fact, anchor=center] () at (-1,2) 
         {\outFactsM{fig_back15_b1}{x/{1}}{0.5in}};
...
This assumption would not hold if we wished to warn the programmer
that a potentially unitialized variable could be used. If we were
analyzing a Haskell or Java program, we also could not make the same
assumption, because all variables are initialized to a known value
(either explicitly or by the compiler). For this language, and our
purposes, we will define \lub as in Table~\ref{tbl_back4}.
