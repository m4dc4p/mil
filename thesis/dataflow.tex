...
node. \inB{x} describes the program's state just prior to entering the
block $B$, while \outB{x} describes the program's state just after
exiting the block.
...
and after every node. Prior to \refNode{lst_back7_assign}, none of the
...
is embedded in a loop.  The fact \inB{lst_back7_assign}, 
$\{a:\bot, b:\bot, c:\bot\}$, represents our lack of information by assigning
each variable the value ``$\bot$'' (``bottom''). After
\refNode{lst_back7_assign}, \outB{lst_back7_assign} has the value 
$\{a:1, b:2, c:\bot\}$, showing that we know #a# is 1, #b# is 2, and we
still do not have information about the value of #c#.

Now consider the value for #c# given by \refNode{lst_back7_true} and
\refNode{lst_back7_false}. \outB{lst_back7_true} says #c# is 4, but
node \outB{lst_back7_false} says #c# equals $3$. To resolve the
situation we assign #c# the value ``$\top$'' (``top'') in
\inB{lst_back7_false}. As opposed to $\bot$, $\top$ says we
...
conflict. Table~\ref{tbl_back2} defines ``\raisebox{.5em}{$\sqcap$}''
