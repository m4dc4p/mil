...
$\mathit{rnd}() + a > b$) or \refNode{lst_back2_false} (when
...
\begin{flalign*}\allowdisplaybreaks[0]
    ...
    \shortintertext{Predecessors of \refNode{lst_back17_test}; Equation~\eqref{eqn_back8}.} 
    \inB{lst_back17_test} &= \outB{lst_back17_assign} \bigwedge \outB{lst_back17_incr} 
    \shortintertext{Equation~\eqref{eqn_back6}.}
    ...
    \shortintertext{Definition of \outB{lst_back17_assign}, and \outB{lst_back17_incr} in Figure~\ref{fig_back11}.}
    ...
    \shortintertext{Equation~\eqref{eqn_back7}.}
    \inB{lst_back17_test} &= \{\factC{i}{0 \lub \top}\}
    \shortintertext{Definition of \lub from Table~\ref{tbl_back4}.} 
    \inB{lst_back17_test} &= \{\factC{i}{\top}\} 
    \shortintertext{Definition of \inB{lst_back17_test}.}
...
Table~\ref{tbl_back5} shows how the \inE and \out sets associated with
each node change with each iteration of the analysis. The control-flow
graph is reproduced in the table as well, to make the relations
between nodes clear. The ``zero'' iteration corresponds to the initial
value for all facts: everything is $\bot$, i.e., unknown. Each
iteration shows the calculated values \inE and \out sets for each
node. Recall that each \inE set is derived the \out sets of the
predecessor nodes. Crucially, the \out set from the \emph{previous}
iteration is used to derive the \inE set for a given
node. \refNode{tbl_back5_test} is the only node with multiple
predecessors.  To derive \inB{tbl_back5_test}, we use
\outB{tbl_back_assign} and \outB{tbl_back_incr} from the
\emph{previous} iteration:
\begin{myfig}
  \setlength{\arraycolsep}{2pt}
%% This style sets options we need
%% below when rendering the CFG next to the
%% table
  \tikzset{dffig/.style={remember picture, overlay, xshift=-0.75in}}
  \begin{math}
  \begin{array}{ll@@{\phantom{X}}ccc@@{\phantom{X}}ccc@@{\phantom{X}}ccc@@{\phantom{X}}ccc@@{\phantom{X}}ccc@@{\phantom{X}}ccc}
    & & %%
      & 0 & & 
      & 1 & & 
      & 2 & & 
    ...
    \begin{tikzpicture}[dffig,yshift=0.1in]
      \node[stmt] (assign) {\begin{minipage}{0.6in} %%
            \tt m = 10 \\n = 1 \\i = 0
          \end{minipage}\labelNode{tbl_back5_assign}}; 
        \node[labelfor=assign] () {\refNode{tbl_back5_assign}};
    \end{tikzpicture}
    & %%
    ...
    \begin{tikzpicture}[dffig] 
      \node[stmt] (test) {\texttt{i < cnt}\labelNode{tbl_back5_test}}; 
      \node[labelfor=test] () {\refNode{tbl_back5_test}};
    \end{tikzpicture} & %%
    \inB{lst_back15_test} & %%
        \bot & \bot & \bot & %% 0 
        \bot & \bot & \bot & %% 1 
    ...
    \begin{tikzpicture}[dffig] 
      \node[stmt] (mult) {\texttt{n = val * m}\labelNode{tbl_back5_mult}};
      \node[labelfor=mult] (mult_label) {\refNode{tbl_back5_mult}};
    \end{tikzpicture}
    & %%
    \inB{lst_back15_mult} & %%
        \bot & \bot & \bot & %% 0 
    ...
    \begin{tikzpicture}[dffig] 
      \node[stmt] (incr) {\texttt{i++}\labelNode{tbl_back5_incr}};
      \node[labelfor=incr] () {\refNode{tbl_back5_incr}};
    \end{tikzpicture}
    & %%
    \inB{lst_back15_incr} & %%
        \bot & \bot & \bot & %% 0 
    ...
    \begin{tikzpicture}[dffig] 
      \node[stmt] (return) {\texttt{return n}\labelNode{tbl_back5_return}}; 
      \node[labelfor=return] () {\refNode{tbl_back5_return}};
    \end{tikzpicture}
    & %%
...
\end{tikzpicture}
  \caption{This table shows the facts calculated for all nodes in our
    example program. After 5 iterations the facts reach a fixed point
    (i.e., they stop changing) and we can see that
    \inB{lst_back15_mult} shows that $m$ is always 10, proving we can
    rewrite the multiplication safely. }
  \label{tbl_back5}
\end{myfig}
...
\afterpage{\clearpage\input{fig_back10}\clearpage}
