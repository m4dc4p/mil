...
\begin{myfig}[tb]
  ...
  \begin{tabular}{cc}
    \subfloat{\input{fig_back11_tbl}\label{fig_back11_tbl}} & 
    \subfloat{\input{fig_back11_cfg}\label{fig_back11_cfg}} \\
...
reached a fixed point? Formally, the question can be answered if our
lattice has \emph{finite height} and the transfer function is
\emph{monotone}.
...
in \setLC. That is, we can define an operator, \sqlt, such that
for all $x$ and $y$ in \setLC:
\begin{equation}
  x \sqlt y\ \text{if and only if}\ x \lub y = y\ \text{and}\ x \neq y.
  \label{eqn_back9}
\end{equation}
That is, $x$ is ``less than'' $y$ only when $x \lub y$ equals
$y$.\footnote{This definition can also be given in terms of ``less
  than or equal to'' by dropping the requirement that $x \neq y$, but
  we choose to simplify and only present \sqlt here.}

We now define the \emph{height} of our lattice. It is the
longest possible ordering of values in \setLC such that:
\begin{equation}
  x_1 \sqlt x_2 \dots \sqlt x_n.
  \label{eqn_back10}
\end{equation}
Equation~\eqref{eqn_back9} shows us that $\top$ is the ``greatest''
element in \setLC and $\bot$ is the ``least.'' In turn, we
know the maximum height of \setLC by Equation~\eqref{eqn_back10}: 3. We
show by contradiction that any longer ordering is impossible:

Given $x_1 \sqlt x_2 \sqlt \dots \sqlt x_4$, such that n = 4. If
$x_4$ is $\top$, then $x_3$ must be an integer or $\bot$. If $x_3$ is
$\bot$, by Equation~\eqref{eqn_back9}, there is no such $x_2$ such
that $x_2 \sqlt \bot$. Therefore, $x_3$ cannot be $\bot$. If $x_3$ is
an integer, again by Equation~\eqref{eqn_back9}, $x_2$ must be
$\bot$. In turn, there is no such $x_1$ such that $x_1 \sqlt \bot$. Therefore,
$x_4$ cannot be $\top$ and in fact, by similar arguments, it cannot exist. We
can show by cases that $n$ (and the height of our lattice) must be 3. $\square$

Now let us address the transfer function. A \emph{monotone} function 
has the following property:
\begin{equation}
  f(x) \sqlt f(y)\ \text{whenever}\x x \sqlt y.
  \label{eqn_back11}
\end{equation}
That is, when a monotone function does not change the ordering between
its inputs. If $x$ is ``less than'' $y$, $f(x)$ will also be ``less
than'' $f(y)$.\footnote{Again, this definition can be given in terms
  $\sqsubseteq$, the ``less than or equal to'' partial ordering.}

When our transfer function is monotone, we know that ``lesser'' facts
will always be translated to ``lesser'' values. Therefore, we know our
facts will either stay constant or get ``bigger.'' They will
\emph{not} vary ``up'' and ``down'' the lattice -- they will only move
in one direction.
