...
Figure \ref{fig_back1} gives the control-flow graph (CFG) for a C
language program fragment. Each node is one statement that will be
executed. Program execution enters the graph from some entry point
(``#O#'' in the diagram). Edges between nodes represent
possible execution paths; the arrows show how statements are executed
in time. Necessarily, the CFG must show \emph{all} possible execution
paths. Each specific run of the program may depend on specific inputs,
which we cannot predict. For that reason we say the CFG approximates
the program's execution. The edges from ``#a > b#'' to
both ``#c = a#'' and ``#c = b#'' show that the program may go to either
statement. Finally, program execution can exit through multiple
nodes, indicated by ``#X#'' in the diagram. 
