...
\let\sp=\ 
\def\removespaces{\afterassignment\removesp\let\next=}
\def\removesp{\ifx\next\sp\let\next=\removespaces%%
\fi\next}  
\protected\def\lab#1/{\ensuremath{\mathbf{\mathtt{\removespaces#1}}}\xspace}
...
\long\def\return#1;{\ensuremath{\ensurett{return\ \removespaces#1}}\xspace}
%% A closure capturing block. k {v1, ..., vN} x: ...
\long\def\ccblock#1(#2)#3:#4/end{\lab#1/\ \ensuremath{\ensurett{\{\ensurett{#2}\}\ #3:\ #4}}}
%% A normal block
\long\def\block#1(#2):#3/end{\lab#1/\ \ensuremath{\ensurett(#2):\ #3}}
%% A goto expression
\long\def\goto#1(#2){\texttt{\textbf{#1}}\ \ensuremath{\ensurett{(#2)}}}
%% An enter expression
