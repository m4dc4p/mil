...
%% LC definition
\newtoks\toksA
\def\lcdef#1(#2)=#3;{\def\arg{#2}\ifx\empty\arg\toksA={\ }%%
  \else\toksA={\ \lcargs #2,/lcargs\ }%%
  \fi%%
  \ensuremath{#1\the\toksA =\ #3}}
\def\lcargs#1,/lcargs{%%
  %% Split head from the list. When
  %% tail is nil, insert ",/lcargs"
  %% so \lcargs gets the delimiter it expects. Otherwise, 
  %% ##2 ends with "," already, so just use ##2 with "/lcargs".
  \def\rest##1,##2/rest{\def\arg{##2}\ifx\empty\arg\global\toks0={,/lcargs}%%
    \else\global\toks0={##2/lcargs}%%
    \fi}%%
  \def\lcarg##1,##2/lcarg{\def\arg{##2}\ifx\empty\arg\toks2={\relax}%%
    \else\toks2={\ }\fi ##1\the\toks2}%%
  \def\arg{#1}%%
  \ifx\empty\arg%%
  \else\lcarg#1,/lcarg\rest#1,/rest\expandafter\lcargs\the\toks0%%
  \fi}
\def\lcapp #1*#2/{\ensuremath{#1\ #2}}
