...
\def\removesp{\foo={\next}\ifcat\noexpand\next\sp\foo={\removespace}%%
 ...
 \fi\the\foo}
\protected\def\lab#1/{\startTTC\textbf{\removespaces #1}\stopTTC}
...
\protected\long\def\clo[#1:#2]{\ensuremath{\{}%%
      ...
      \else\lab #1/%%
        ...
        \else\ensurett{:\ }%%
        \fi%%
      \fi\ensuremath{\ensurett{#2}}\ensuremath{\}}\xspace}
%% Binding statement: v <- {...}
...
\protected\long\def\return#1;{\startTTC return\ \removespaces #1\stopTTC\xspace}
%% A closure capturing block. k {v1, ..., vN} x: ...
\protected\long\def\ccblock#1(#2)#3:{\lab#1/\ \ensuremath{\{\startTTC #2\stopTTC\}}\ \ensurett{#3:}}
%% A normal block
\protected\long\def\block#1(#2):{\lab #1/\ \ensuremath{(\startTTC #2\stopTTC)}\ensurett{:}}
%% A goto expression
\protected\long\def\goto#1(#2){\texttt{\textbf{\removespaces #1}}\ \ensuremath{(\ensurett{#2})}}
%% An enter expression
\protected\long\def\app#1*#2/{\texttt{\removespaces #1@#2}}
\protected\def\bind{\texttt{<-}\xspace}
\protected\def\prim#1(#2){\lab #1/*\ensuremath{(\startTTC #2\stopTTC)}}
