...
\newcommand{\lub}{\ifmmode\sqcap\else\raisebox{.1em}{\ensuremath{\sqcap}}\fi\xspace}
...
{\def\.{\global\let\sp= } \. } 
%% Disable fancyvrb's space macro. Use \begingroup to keep change
%% local.
\makeatletter
\def\removespaces{\begingroup\removespaceswork}
\def\removespaceswork{\futurelet\next\removesp}
\newtoks\rmspace 
\def\removesp{\ifcat\noexpand\next\sp%%
    \rmspace={\afterassignment\removespaceswork\let\next= }%%
    \else\rmspace={\endgroup}%%
    \fi\the\rmspace} \makeatother 
\def\lab#1/{\ensurett{\textbf{\removespaces #1}}}
%% Constructs a closure: l { v1, ..., vN }
\long\def\mkclo[#1:#2]{\text{\texttt{\textbf{#1}}}\ \ensuremath{\ensurett{\{#2\}}}\xspace}
%% Tuple version of closurs: {l: v1, ..., vN}.
\long\def\clo[#1:#2]{\ensuremath{\{}\ensurett{%%
      \ifx#1\relax%%
      \else\lab #1/%%
        \ifx#2\relax%%
        \else:\ %%
        \fi%%
      \fi#2\ensuremath{\}}}\xspace}
%% Binding statement: v <- {...}
\long\def\binds#1<-#2;{\ensurett{\removespaces #1<-#2}\xspace}
...
\long\def\return#1;{\ensurett{return\ \removespaces #1}\xspace}
%% A closure capturing block. k {v1, ..., vN} x: ...
\long\def\ccblock#1(#2)#3:#4/end{\lab#1/\ \ensuremath{\{}\ensurett{#2}\ensuremath{\}}\ \ensurett{#3:\ #4}}
%% A normal block
\long\def\block#1(#2):#3/end{\lab #1/\ \ensuremath{(}\ensurett{#2}\ensuremath{)}:\ #3}
%% A goto expression
\long\def\goto#1(#2){\texttt{\textbf{\removespaces #1}}\ \ensuremath{(}\ensurett{#2}\ensuremath{)}}
%% An enter expression
\long\def\app#1*#2/{\texttt{\removespaces #1@#2}}
\def\bind{\texttt{<-}\xspace}
\def\var#1/{\texttt{\removespaces #1}\xspace}
...
\DefineVerbatimEnvironment{MILVerb}{Verbatim}{commandchars=\\\[\],%% 
  numberblanklines=false}
