...
\def\removesp{\foo={\next}\ifcat\noexpand\next\sp\foo={\removespace}%%
 ...
 \fi\the\foo}
\protected\def\lab#1/{\ensurett{\textbf{\removespaces #1}}}
%% Constructs a closure: l { v1, ..., vN }
\protected\long\def\mkclo[#1:#2]{\text{\texttt{\textbf{#1}}}\ \ensuremath{\ensurett{\{#2\}}}\xspace}
%% Tuple version of closurs: {l: v1, ..., vN}.
\protected\long\def\clo[#1:#2]{\ensuremath{\{}\ensurett{%%
      \ifx#1\relax%%
      \else\lab #1/%%
        \ifx#2\relax%%
        \else:\ %%
...
%% Binding statement: v <- {...}
\protected\long\def\binds#1<-#2;{\ensurett{\removespaces #1<-#2}\xspace}
...
\begingroup\catcode`\!=\active \lccode`\!=`\< \lccode`\~=`\- 
  \catcode`\ =\active\lowercase{\endgroup\protected\def\vbinds#1!~#2;}{\binds#1<-#2;}
%% Return statement: return ... ;
\protected\long\def\return#1;{\ensurett{return\ \removespaces #1}\xspace}
%% A closure capturing block. k {v1, ..., vN} x: ...
\protected\long\def\ccblock#1(#2)#3:#4/end{\lab#1/\ \ensuremath{\{}\ensurett{#2}\ensuremath{\}}\ \ensurett{#3:\ #4}}
%% A normal block
\protected\long\def\block#1(#2):#3/end{\lab #1/\ \ensuremath{(}\ensurett{#2}\ensuremath{)}:\ #3}
%% A goto expression
\protected\long\def\goto#1(#2){\texttt{\textbf{\removespaces #1}}\ \ensuremath{(}\ensurett{#2}\ensuremath{)}}
%% An enter expression
\protected\long\def\app#1*#2/{\texttt{\removespaces #1@#2}}
\protected\def\bind{\texttt{<-}\xspace}
\protected\def\var#1/{\texttt{\removespaces #1}\xspace}
