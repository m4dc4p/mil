...
import Habit.Compiler.Stack.Machine (Instr(Note))
...
dump_typed_mod m =
  ...
  where
    ...
    ppImplDecl (ImplDecl { decl_name = name
                         ...
                         , decl_body = body}) = text "decl_name: " <+> pp name $+$
...
save_typed_mod_dump (AcyclicSCC mo) =
  do let m = the_ast (get_pass mo)
     ...
     save_file (mod_name_to_file (mod_name m) <.> "dump")
                   $ show $ run_pp_with env $ dump_typed_mod m
#ifdef DEBUG
     save_file (mod_name_to_file (mod_name m) <.> ".s.hs")
                   $ (concatMap (\i -> show i ++ "\n") (compile supply m))
#else
     let isNote (Note _) = True
         isNote _ = False
     save_file (mod_name_to_file (mod_name m) <.> ".s.hs")
                $ (concatMap (\i -> show i ++ "\n") (filter (not . isNote) . compile supply $ m))
#endif
